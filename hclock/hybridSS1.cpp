...
void prediction( SubReq (&reqs)[DEVNUM] )
{
    int i;
    ...
    for( i=0; i< DEVNUM; i++ )
    {
        ...
        else
        {
            ...
            case 1:
                ...
                parts[i].Tseek = fastHddSeek( blkDist );
                parts[i].Tdisk = parts[i].Tseek + reqs[i].reqSize * parts[i].Ttran;
                break;
            case 2:
                blkDist = (reqs[i].reqStart - parts[i].lastBlkno);
                parts[i].lastBlkno = reqs[i].reqStart + reqs[i].reqSize - 1;
                parts[i].Tseek = slowHddSeek( blkDist );
                parts[i].Tdisk = parts[i].Tseek + reqs[i].reqSize * parts[i].Ttran;
                break;
            case 3:
                //parts[i].Tseek = 0;
                //parts[i].Ttran = 0.14;
                parts[i].Tdisk = parts[i].Ttran;
                break;
            default:
...
void computePG( SubReq (&reqs)[DEVNUM], unsigned long prefetch, long reqBlkNum, int appType )
{
    ...
    for( i=0; i < DEVNUM; i++ )
    {
        ...
        if( appType == 0 )
        {
            ...
            if( parts[i].bufPart.blkNum >= (parts[i].ranMax - parts[i].ranMin) ) td = 1;
            else td = (double)parts[i].bufPart.blkNum / (double)(parts[i].ranMax - parts[i].ranMin);
            parts[i].ranPG = ( 1 - td ) * parts[i].Tdisk;
        }
        else
        {
            //prefetch = prefetchEnd, 非0表示执行过预取
            if( prefetch ) parts[i].seqPG = ((parts[i].Tseek + parts[i].Ttran * reqs[i].reqSize ) * reqBlkNum ) / (reqs[i].reqSize * DEVNUM);
        }
    }
...
{
    int i, index = pid;
    double minPG = parts[pid].ranPG*parts[pid].lowBound;
    ...
    if( apptype == 0 )
    {
        for( i=0; i < DEVNUM; i++ )
        {
            ...
            if( parts[i].ranPG*parts[i].lowBound < minPG )
            {
                index = i;
                minPG = parts[i].ranPG*parts[i].lowBound;
            }
        }
    }
    else
    {
        ...
        {
            if( parts[i].bufPart.blkNum <= parts[i].lowBound ) continue;
            if( parts[i].seqPG*parts[i].lowBound < minPG )
            {
                index = i;
                minPG = parts[i].seqPG*parts[i].lowBound;
            }
        }
    }
            ...
            else
            {
                ...
                else
                {
					...
					if( tpBuf == parts[i].current ){
					  for(k=0;k<DEVNUM;k++){
                        printf("dev-%d blkNum=%d,lowBound=%d,bio_num=%d",k,parts[k].bufPart.blkNum,parts[k].lowBound,bio_num[k]);
					  printf("dev-%d:idtype%d avgPerf:%f ranPG:%f, seqPG:%f\n",itapp->second.type,k,parts[k].avgPerf,parts[k].ranPG,parts[k].seqPG);
		               }
