...
void updateBlkno( unsigned long blkno, Buf *pBuf, BufList *list )
{
    if( !lookup(pBuf->blkno,list->table) )
    {
        cout<<pBuf->blkno<<endl;
        printf("error: update a non-exist buf block\n");
        exit(1);
    }
    if( !del(pBuf->blkno,list->table) )
    {
        printf("error: delete fail in update operations\n");
        exit(1);
    }
    insert( blkno, pBuf, list->table);
...
int main()
{
    ...
    while(1)
    {
        ...
        cout<<"7 find(blk); 8 update(blk-old, blk-new)"<<endl;
        cin>>op;
        cin>>blkno;
        switch(op)
        {
            ...
            case 8:
                cin>>blkno2;
                cout<<blkno<<", "<<blkno2<<endl;
                pBuf = isBlkInList( blkno, &list );
                updateBlkno(blkno2, pBuf, &list );
                break;
            default:
