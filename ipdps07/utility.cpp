...
#define SegmentNum 50  //每个segment大小是cache的%
...
static int SegLength = 0;          //segment的长度， bufSize / 50
...
void doInit()
{
    ...
    n = 0;
    //SegLength = BUFSIZE / SegmentNum;
    assert( SegLength != 0 );
    tnum = SegmentNum / DEVNUM;          //12?
    for( i = 0; i < DEVNUM; i++ )
    {
        for( j = 0; j < tnum; j++ )
        {
            ...
            parts[i].insertSegToTail(pSeg);
        }
    }
    tnum = SegmentNum - tnum * DEVNUM;
    for( j = 0; j < tnum; j++ )
    {
        pSeg = new Segment(SegLength);
        for( k = 0; k < SegLength; k++ )
...
vector<double> hybridRead( vector<SubReq> (&ioreqs)[DEVNUM] )
{
    int i, j, n;
    SubReq *psubreq;
    double delay;
    ...
    for( i=0; i<DEVNUM; i++ )
    {
        ...
        if( n != 0 )
        {
            ...
            parts[i].averageTime( (delay / n) );
            response.push_back( (delay / n) );
        }
        else response.push_back( -2.0 );   //负数表示没有读取
    }
    now = maxtime;
...
Buf *isHitInCache( unsigned long blkno, Segment* &pSeg, int &pno )
{
    int i;
    Buf *pBuf;
    ...
    for( i = 0; i < DEVNUM; i++ )
    {
        pBuf = parts[i].isBufInPartition( blkno, pSeg );
        if( pBuf != NULL )
        {
            pno = i;
            return pBuf;
...
int main(int argc, char *argv[] )
{
    ...
    int i, j, n, RetVal, windows=0, devno, id, pno, ghostHits[DEVNUM][GhostLength];
    double tdouble, td2;
    ...
    if (argc != 5)
    {
        fprintf(stderr, "usage: %s <设备参数文件> <负载文件> <StripeUnit> <CacheSize> \n", argv[0]);
        exit(1);
    }
    ...
    while( getline( fin, line) )
    {
        ...
        for( j = 0 ; j<reqBlkNum; j++ )
        {
            ...
            pBuf = isHitInCache(reqBlk, pSeg, pno);
            if( pBuf != NULL )
            {
                totalHits++;
                if( devno == pno )
                {
                    pSeg->mu += parts[devno].avgDelay;
                    parts[devno].moveBufToHead( pBuf );
                }
                if( reqType == 1 )
                {
                    if( pBuf->dirty == 0 )
            ...
            {
                i = parts[devno].ghost.isHitInGhost( reqBlk );
                if( i != -1 )
                {
                    ghostHits[devno][i]++;
                }
                pBuf = parts[devno].deleteBufFromTail();
                assert( pBuf != NULL );
                if( (devno == pno) && (pBuf->valid != 0) )
                {
                    parts[devno].ghost.insertBufToGhost( pBuf->blkno );
                }
                if( pBuf->dirty )
                {
                    do_write( pBuf->blkno );
                ...
                pBuf->valid = 1;
                pBuf->dirty = 0;
                parts[devno].insertBufToHead( pBuf );
                if( reqType == 0 )
                {
        ...
        for( i = 0; i < DEVNUM; i++ )
        {
            for( j = 0; j < GhostLength; j++ )
            {
                if( ghostHits[i][j] != 0 )   //更新mu
                {
                    assert( responses[i] > 0 );
                    parts[i].ghost.updateMuInGhost( responses[i], ghostHits[i][j], j );
                }
                if( j == 0 )    //是否发生条件改变
                {
                    if( max == i ) tag = 1;
                    ...
                    {
                        if( max == -1 )      //第一个，吸收空闲的segment
                        {
                            max = i;
                            tag = 1;
                        }
                        if( parts[i].ghost.getCmu() > parts[max].ghost.getCmu() )
                        {
                            max = i;
                            tag = 1;
                        }
        ...
        while( tag == 1 )
        {
            if( (parts[max].ghost.getCmu() - minv) > DELTA )   //更新
            {
                j = -1;
                for( i = 0; i < DEVNUM; i++ )
                {
                    if( i == max ) continue;   //customer和supplier不能是同一个
                    if( (parts[i].bufPart.tail->mu < tdouble) || ( j == -1) )
                    {
                        j = i;
                        tdouble = parts[i].bufPart.tail->mu;
                    }
                }
                min = j;
                minv = tdouble;
            ...
            if( (parts[max].ghost.getCmu() - minv) > DELTA )    //判断
            {
                pSeg = parts[min].deleteSegFromTail();
                parts[max].insertSegToTail(pSeg);
                pSeg->mu = parts[max].ghost.getCmu();
                parts[max].ghost.moveSegToLastGhost();
            }
            j = -1;
            for( i = 0; i < DEVNUM; i++ )    //找新的max
            {
                if( (parts[i].ghost.getCmu() > tdouble) || ( j == -1) )
                {
                    j = i;
                    tdouble = parts[i].ghost.getCmu();
                }
            }
            max = j;
            j = -1;
            for( i = 0; i < DEVNUM; i++ )       //找新的min
            {
                if( i == max ) continue;   //customer和supplier不能是同一个
                if( (parts[i].bufPart.tail->mu < tdouble) || ( j == -1) )
                {
            ...
            minv = tdouble;
            assert( min != max );
            if( (parts[max].ghost.getCmu() - minv) > DELTA ) tag = 1;   //是否需要继续
            else tag = 0;
        }
    }//next request
